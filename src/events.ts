import type { EventHandler, Event } from './types'
...
export default class EventEmitter {
   ...
   * @memberof EventEmitter
   */
  #createEvent (type: string, data?: any, once: boolean = false): Event {
  ...
  on (type: string, handler: EventHandler): boolean {
    ...
    if (handlers == null || handlers.length === 0) handlers = this.#_eventHandlers[type] = []
  ...
  has (type: string, handler?: EventHandler): boolean {
    ...
    if (handler == null || !this.#isValidHandler(handler)) return true
